"""option-b-new-messages.py - multi-turn chat, Option B: only the latest run.

Five scripted turns through the same agent. After each run, the NEXT run is
handed `result.new_messages()`: only what this run produced. The model sees
just the previous exchange every turn, so "my unit" and "tomorrow" stop
resolving one turn after the demo starts looking fine.

The twin file, option-a-all-messages.py, is byte-identical except for the one
marked line near the bottom. Run both and compare the traces.

Run:  python option-b-new-messages.py
"""
import os
